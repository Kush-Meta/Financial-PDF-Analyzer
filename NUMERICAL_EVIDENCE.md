...
Exact aliases live in `ALIASES`. For example, total liabilities must not match the combined liabilities-and-equity row. A bare “Services” row is accepted as services revenue only under a recognized sales/revenue section, so a services cost row cannot substitute for it. Operating cash flow accepts both Apple-style “cash generated by operating activities” and Microsoft-style “net cash from operations”; investing or financing cash rows do not qualify. Statement headings must match the recognized `STATEMENT` pattern (for example `Consolidated Statements of Income`); issuer-HTML labels such as `Financial Statements: Income Statements` remain unsupported and must abstain.
