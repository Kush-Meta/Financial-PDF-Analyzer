...
Checked formulas now also include **net margin** (`net_income / revenue * 100`), **return on equity** (`net_income / shareholders_equity * 100`), **free cash flow** (`operating_cash_flow - abs(capital_expenditure)`), and **year-over-year growth** for any supported statement metric (not only revenue). Adjusted or organic free-cash-flow requests remain unsupported.
...
Exact aliases live in `ALIASES`. For example, total liabilities must not match the combined liabilities-and-equity row. A bare “Services” row is accepted as services revenue only under a recognized sales/revenue section, so a services cost row cannot substitute for it. Operating cash flow accepts both Apple-style “cash generated by operating activities” and Microsoft-style “net cash from operations”; investing or financing cash rows do not qualify. Statement headings match the recognized `STATEMENT` pattern, including `Consolidated Statements of Income`, issuer-HTML `Income Statements` / `Cash Flows Statements`, and a `Financial Statements:` prefix. Segment or adjusted income-statement headings remain unsupported and must abstain. Capex aliases include Microsoft’s “additions to property and equipment”.

Operating margin is `operating_income / revenue * 100`. Net margin is `net_income / revenue * 100`. Return on equity is `net_income / shareholders_equity * 100` using year-end equity from the same filing. Metric growth is `(current - prior) / prior * 100` for a single supported metric across two named years. Free cash flow is `operating_cash_flow - abs(capital_expenditure)`. Margin and growth formulas require compatible inputs from the same statement and currency (except ROE, which joins income and year-end equity). Free cash flow requires both cash-flow rows from the same statement and currency. Percentages display two decimal places; the research record retains the decimal result and inputs. Capital expenditure output is explicitly an outflow magnitude, with `abs(reported_cash_flow)` recorded separately from the original signed cell.
