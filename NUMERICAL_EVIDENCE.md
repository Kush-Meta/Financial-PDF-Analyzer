...
Checked formulas now also include **net margin** (`net_income / revenue * 100`) and **free cash flow** (`operating_cash_flow - abs(capital_expenditure)`), both from accepted statement cells with the same currency and table. Adjusted or organic free-cash-flow requests remain unsupported.
...
Operating margin is `operating_income / revenue * 100`. Net margin is `net_income / revenue * 100`. Revenue growth is `(current_revenue - prior_revenue) / prior_revenue * 100`. Free cash flow is `operating_cash_flow - abs(capital_expenditure)`. Margin and growth formulas require compatible inputs from the same statement and currency, with a positive revenue denominator. Free cash flow requires both cash-flow rows from the same statement and currency. Percentages display two decimal places; the research record retains the decimal result and inputs. Capital expenditure output is explicitly an outflow magnitude, with `abs(reported_cash_flow)` recorded separately from the original signed cell.
