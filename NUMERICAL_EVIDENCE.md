...
A second-issuer offline fixture covers Alphabet’s FY2024 consolidated income and cash-flow statement layouts (see `tests/fixtures/alphabet-2024-statements.json`), including questions such as:

- “What was Alphabet's revenue in fiscal 2024?”
- “What was income from operations in fiscal 2024?”
- “What was net cash provided by operating activities in fiscal 2024?”
- “What was operating margin in fiscal 2024?”

A third-issuer fixture covers Microsoft’s FY2025 Income Statements / Cash Flows Statements layout (see `tests/fixtures/microsoft-fy2025-statements.json`), including issuer-HTML headings and:

- “What was Microsoft's total revenue in fiscal 2025?”
- “What was net cash from operations in fiscal 2025?”
- “What was free cash flow in fiscal 2025?”
- “What was net margin in fiscal 2025?”

Checked formulas now also include **net margin** (`net_income / revenue * 100`), **return on equity** (`net_income / shareholders_equity * 100`), **current ratio** (`current_assets / current_liabilities`), **free cash flow** (`operating_cash_flow - abs(capital_expenditure)`), and **year-over-year growth** for any supported statement metric (not only revenue). Adjusted or organic free-cash-flow requests remain unsupported.
...
| Balance sheet | `cash_and_equivalents`, `current_assets`, `current_liabilities`, `total_assets`, `total_liabilities`, `shareholders_equity` |
| Annual cash flow | `operating_cash_flow`, `capital_expenditure`; a matching `net_income` row can also supply reported net income |

Exact aliases live in `ALIASES`. For example, total liabilities must not match the combined liabilities-and-equity row. Total current assets must not match “other current assets”. A bare “Services” row is accepted as services revenue only under a recognized sales/revenue section, so a services cost row cannot substitute for it. Revenue aliases include Coca-Cola-style `net operating revenues`. Operating cash flow accepts both Apple-style “cash generated by operating activities” and Microsoft-style “net cash from operations”; investing or financing cash rows do not qualify. Statement headings match the recognized `STATEMENT` pattern, including `Consolidated Statements of Income`, issuer-HTML `Income Statements` / `Cash Flows Statements`, and a `Financial Statements:` prefix. Segment or adjusted income-statement headings remain unsupported and must abstain. Capex aliases include Microsoft’s “additions to property and equipment”.

Operating margin is `operating_income / revenue * 100`. Net margin is `net_income / revenue * 100`. Return on equity is `net_income / shareholders_equity * 100` using year-end equity from the same filing. Current ratio is `current_assets / current_liabilities` using year-end balances from the same balance sheet (displayed as a multiple, e.g. `1.03×`). Metric growth is `(current - prior) / prior * 100` for a single supported metric across two named years. Free cash flow is `operating_cash_flow - abs(capital_expenditure)`. Margin and growth formulas require compatible inputs from the same statement and currency (except ROE, which joins income and year-end equity). Current ratio and free cash flow require both inputs from the same statement and currency. Percentages display two decimal places; the research record retains the decimal result and inputs. Capital expenditure output is explicitly an outflow magnitude, with `abs(reported_cash_flow)` recorded separately from the original signed cell.
...
5. Supported facts, formulas, and explicit evidence abstentions return before the planner LLM, embeddings, reranking, or answer generation. Checked annual figures therefore work even when Ollama is unavailable. Peer follow-ups and unsupported document questions still use the planner.
6. An unsupported question, an absent statement, or a scan exceeding its budget continues to normal planning, semantic retrieval and model analysis. `PageContextRetriever` expands ranked excerpts within its existing context budget. The app passes `check_numbers=False` to this fallback: a ranked subset cannot earn a checked status after the complete scan was skipped. Any generated interpretation is marked `not_checked`.
