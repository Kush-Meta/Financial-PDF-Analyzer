...
def _request(question, sources):
    ...
    allowed = set("what was were is are how much did the this company companys reported report annual fiscal year years end at in for from to and of amount amounts give show me please calculate compute cite statement filing document usd million millions billion billions thousand thousands dollars outflow magnitude on during with sources source percentage percent free cash flow flows fcf net margin margins".split())
    if set(query.split()) - allowed:
        return None
    if formula in ("operating_margin", "net_margin", "free_cash_flow") and len(metrics) != 2:
        return None
...
def quantitative_answer(question, sources):
    ...
    for metric in metrics:
        for year in years:
            ...
            # Margin formulas need the income statement row, not a cash-flow
            # restatement of net income, so the inputs share one table.
            if formula in ("operating_margin", "net_margin"):
    ...
    if formula:
        ...
        if formula == "free_cash_flow":
            ...
            periods = years if formula in ("operating_margin", "net_margin") else [years[-1]]
            for year in periods:
                if formula == "operating_margin":
                    ...
                    numerator = pairs[("revenue", years[-1])]
                    denominator = pairs[("revenue", years[0])]
                ...
                with localcontext() as context:
                    context.prec = 28
                    result = (top / bottom if formula in ("operating_margin", "net_margin") else (top - bottom) / bottom) * 100
                period = f"fiscal {year}" if formula in ("operating_margin", "net_margin") else f"fiscal {years[0]} to {years[-1]}"
                title = formula.replace("_", " ").capitalize()
                lines.append(
                    f"**{title} ({period}): {result:.2f}%** [{numerator['source_id']}]\n\n"
                    f"{expression.replace('_', ' ')} = " +
                    (f"{_money(top, numerator['currency'])} ÷ {_money(bottom, denominator['currency'])} × 100"
                     if formula in ("operating_margin", "net_margin") else
                     f"({_money(top, numerator['currency'])} − {_money(bottom, denominator['currency'])}) ÷ "
                     f"{_money(bottom, denominator['currency'])} × 100") + "."
                )
                verification["calculations"].append({
                    "metric": formula, "year": year, "formula": expression,
