...
def _request(question, sources):
    ...
    allowed = set("what was were is are how much did the this company companys reported report annual fiscal year years end at in for from to and of amount amounts give show me please calculate compute cite statement filing document usd million millions billion billions thousand thousands dollars outflow magnitude on during with sources source percentage percent free cash flow flows fcf net margin margins return equity roe growth".split())
    if set(query.split()) - allowed:
        return None
    if formula in ("operating_margin", "net_margin", "free_cash_flow", "roe") and len(metrics) != 2:
        return None
