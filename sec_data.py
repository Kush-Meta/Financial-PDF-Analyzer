...
class SecClient:
    def __init__(self, user_agent=None, budget=16, deadline=75):
        self.user_agent = user_agent or os.environ.get(
            "SEC_USER_AGENT", "FinRead research https://github.com/Kush-Meta/Financial-PDF-Analyzer")
        ...
        for attempt in range(2):
            with _lock:
                ...
                if self.remaining <= 0 or time.monotonic() >= self.deadline:
                    raise ResearchError("SEC research reached its request or time limit. Try fewer companies.")
                time.sleep(max(0, .25 - (time.monotonic() - _last_request)))
                self.remaining -= 1
                ...
                try:
                    ...
                    if len(raw) > MAX_RESPONSE:
                        raise ResearchError("SEC response exceeded the research size limit.")
                    data = json.loads(raw)
                    ...
                    if exc.code == 403:
                        raise ResearchError("SEC denied access from this network. Configure SEC_USER_AGENT with your application and contact, or retry from a permitted network.") from exc
                    if exc.code not in (429, 500, 502, 503, 504) or attempt:
                        raise ResearchError(f"SEC data is unavailable (HTTP {exc.code}). Try again later.") from exc
                except (URLError, TimeoutError, OSError, json.JSONDecodeError) as exc:
                    raise ResearchError("SEC could not be reached or returned unreadable data. Check the connection and retry.") from exc
            time.sleep(1)
        raise ResearchError("SEC data is temporarily unavailable.")
